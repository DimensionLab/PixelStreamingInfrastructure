...
class Streamer {
  ...
  addSFUPlayer(sfuPlayerId) {
    if (!!this.SFUPlayerId && this.SFUPlayerId != sfuPlayerId) {
      console.error(
        `Streamer ${this.id} already has an SFU ${this.SFUPlayerId}. Trying to add ${sfuPlayerId} as SFU.`
      );
      return;
    }
  ...
  setSFUPlayerComponent(playerComponent) {
    if (!this.isSFU()) {
      console.error(
        `Trying to add an SFU player component ${playerComponent.id} to streamer ${this.id} but it is not an SFU type.`
      );
      return;
    }
  ...
  getSFUPlayerComponent() {
    if (!this.isSFU()) {
      console.error(
        `Trying to get an SFU player component from streamer ${this.id} but it is not an SFU type.`
      );
      return null;
    }
...
class Player {
    ...
    if (this.streamerId && streamers.has(this.streamerId)) {
      if (this.type == PlayerType.SFU) {
        ...
        if (streamer.getSFUPlayerId() != this.id) {
          console.error(
            `Trying to unsibscribe SFU player ${this.id} from streamer ${
              streamer.id
            } but the current SFUId does not match (${streamer.getSFUPlayerId()}).`
          );
        } else {
  ...
  setSFUStreamerComponent(streamerComponent) {
    if (!this.isSFU()) {
      console.error(
        `Trying to add an SFU streamer component ${streamerComponent.id} to player ${this.id} but it is not an SFU type.`
      );
      return;
    }
  ...
  getSFUStreamerComponent() {
    if (!this.isSFU()) {
      console.error(
        `Trying to get an SFU streamer component from player ${this.id} but it is not an SFU type.`
      );
      return null;
    }
...
function onSFUMessageStartStreaming(sfuPlayer, msg) {
  ...
  if (streamers.has(sfuStreamerComponent.id)) {
    console.error(
      `SFU ${sfuStreamerComponent.id} is already registered as a streamer and streaming.`
    );
    return;
  }
...
function onSFUMessageStopStreaming(sfuPlayer, msg) {
  ...
  if (!streamers.has(sfuStreamerComponent.id)) {
    console.error(
      `SFU ${sfuStreamerComponent.id} is not registered as a streamer or streaming.`
    );
    return;
  }
...
sfuServer.on("connection", function (ws, req) {
  ...
  let streamerComponent = new Streamer(
  ...
  ws.on("message", (msgRaw) => {
    ...
    if (!sfuPlayer) {
      console.error(
        `Received a message from an SFU not in the player list ${playerId}`
      );
